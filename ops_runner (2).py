...
# ── Allowlist config ──────────────────────────────────────────────────────────
# Path to the file that lists permitted scripts — one absolute path per line.
# Lines starting with '#' and blank lines are ignored.
# Edit this path if you want to keep the file elsewhere.
...
def _load_allowlist():
    """
    Read ALLOWLIST_FILE and return a set of normalised absolute paths.
    Re-read on every request so changes take effect without restarting Flask.
    Returns (allowed_set, error_message).  error_message is None on success.
    """
    if not os.path.exists(ALLOWLIST_FILE):
        return None, (
            f"Allowlist file not found: {ALLOWLIST_FILE}\n"
            "Create it and add one allowed script path per line."
        )
    try:
        with open(ALLOWLIST_FILE, "r", encoding="utf-8") as fh:
            lines = fh.readlines()
    except OSError as e:
        return None, f"Cannot read allowlist file: {e}"
    ...
    for line in lines:
        ...
        if not line or line.startswith("#"):   # skip blanks and comments
            continue
        allowed.add(os.path.normpath(line))    # normalise so /a/../b == /b
    return allowed, None


def _is_allowed(script_path, allowed_set):
    """Return True if the normalised script_path is in the allowlist."""
    return os.path.normpath(script_path) in allowed_set
...
def run_script():
    ...
    # ── Allowlist check — must pass before anything else runs ─────────────────
    allowed_set, al_err = _load_allowlist()
    if al_err:
        return render_template("index.html",
                               script_error=f"Allowlist error: {al_err}")
    if not _is_allowed(script_path, allowed_set):
        # Tell the user clearly — do NOT reveal what IS in the allowlist
        return render_template("index.html",
                               script_error=(
                                   f"⛔  Not allowed: {script_path}\n\n"
                                   f"This script is not in the allowlist.\n"
                                   f"To permit it, add its full path to:\n"
                                   f"  {ALLOWLIST_FILE}"
                               ))
